package cool.muyucloud.croparia.neoforge;

import cool.muyucloud.croparia.data.crop.CompatCrop;
import cool.muyucloud.croparia.data.crop.CropType;
import cool.muyucloud.croparia.registry.Crops;
import org.jetbrains.annotations.Nullable;

import java.util.Map;

public class CompatCrops {
    @Nullable
    public static final CompatCrop ALUMINUM = Crops.compatCrop("aluminum", "#c:ingots/aluminum", 0x9E9E9E, 3, CropType.CROP, Map.of(
        "modern_industrialization", "item.modern_industrialization.aluminum_ingot"
    ));
    @Nullable
    public static final CompatCrop ANTIMONY = Crops.compatCrop("antimony", "#c:ingots/antimony", 0x8A8A8A, 3, CropType.CROP, Map.of(
        "modern_industrialization", "item.modern_industrialization.antimony_ingot"
    ));
    @Nullable
    public static final CompatCrop BARONYTE = Crops.compatCrop("baronyte", "#c:ingots/baronyte", 0xE56544, 3, CropType.CROP, Map.of(
        "aoa3", "item.aoa3.baronyte_ingot"
    ));
    @Nullable
    public static final CompatCrop BLAZIUM = Crops.compatCrop("blazium", "#c:ingots/blazium", 0xFCEB6F, 3, CropType.CROP, Map.of(
        "aoa3", "item.aoa3.blazium_ingot"
    ));
    @Nullable
    public static final CompatCrop BRONZE = Crops.compatCrop("bronze", "#c:ingots/bronze", 0xC48553, 3, CropType.CROP, Map.of(
        "mekanism", "item.mekanism.ingot_bronze"
    ));
    @Nullable
    public static final CompatCrop CERTUS = Crops.compatCrop("certus", "#c:gems/certus_quartz", 0xB8D8FC, 3, CropType.CROP, Map.of(
        "ae2", "item.ae2.certus_quartz_crystal"
    ));
    @Nullable
    public static final CompatCrop ELECANIUM = Crops.compatCrop("elecanium", "#c:ingots/elecanium", 0x34ACDE, 3, CropType.CROP, Map.of(
        "aoa3", "item.aoa3.elecanium_ingot"
    ));
    @Nullable
    public static final CompatCrop EMBERSTONE = Crops.compatCrop("emberstone", "#c:ingots/emberstone", 0xF17F22, 3, CropType.CROP, Map.of(
        "aoa3", "item.aoa3.emberstone_ingot"
    ));
    @Nullable
    public static final CompatCrop FLUIX = Crops.compatCrop("fluix", "#c:gems/fluix", 0x8F5CCB, 3, CropType.CROP, Map.of(
        "ae2", "item.ae2.fluix_crystal"
    ));
    @Nullable
    public static final CompatCrop GHASTLY = Crops.compatCrop("ghastly", "#c:ingots/ghastly", 0xF8FC9C, 3, CropType.CROP, Map.of(
        "aoa3", "item.aoa3.ghastly_ingot"
    ));
    @Nullable
    public static final CompatCrop GHOULISH = Crops.compatCrop("ghoulish", "#c:ingots/ghoulish", 0x7EA8FC, 3, CropType.CROP, Map.of(
        "aoa3", "item.aoa3.ghoulish_ingot"
    ));
    @Nullable
    public static final CompatCrop LEAD = Crops.compatCrop("lead_conventional", "#c:ingots/lead", 0x6F6B77, 3, CropType.CROP, Map.of(
        "modern_industrialization", "item.modern_industrialization.lead_ingot",
        "mekanism", "item.mekanism.ingot_lead"
    ));
    @Nullable
    public static final CompatCrop LIMONITE = Crops.compatCrop("limonite", "#c:ingots/limonite", 0xE79353, 3, CropType.CROP, Map.of(
        "aoa3", "item.aoa3.limonite_ingot"
    ));
    @Nullable
    public static final CompatCrop LUNAR = Crops.compatCrop("lunar", "#c:ingots/lunar", 0xA32F9D, 3, CropType.CROP, Map.of(
        "aoa3", "item.aoa3.lunar_ingot"
    ));
    @Nullable
    public static final CompatCrop MONAZITE = Crops.compatCrop("monazite", "#c:dusts/monazite", 0xFCC4B3, 3, CropType.CROP, Map.of(
        "modern_industrialization", "item.modern_industrialization.monazite_dust"
    ));
    @Nullable
    public static final CompatCrop MYSTITE = Crops.compatCrop("mystite", "#c:ingots/mystite", 0xB3FCC4, 3, CropType.CROP, Map.of(
        "aoa3", "item.aoa3.mystite_ingot"
    ));
    @Nullable
    public static final CompatCrop NICKEL = Crops.compatCrop("nickel", "#c:ingots/nickel", 0x8F9E9A, 3, CropType.CROP, Map.of(
        "modern_industrialization", "item.modern_industrialization.nickel_ingot"
    ));
    @Nullable
    public static final CompatCrop IRIDIUM = Crops.compatCrop("iridium", "#c:ingots/iridium", 0x8F9E9A, 3, CropType.CROP, Map.of(
        "modern_industrialization", "item.modern_industrialization.iridium_ingot"
    ));
    @Nullable
    public static final CompatCrop SALT = Crops.compatCrop("salt", "#c:dusts/salt", 0x8F9E9A, 3, CropType.CROP, Map.of(
        "modern_industrialization", "item.modern_industrialization.salt_dust"
    ));
    @Nullable
    public static final CompatCrop SHYRESTONE = Crops.compatCrop("shyrestone", "#c:ingots/shyrestone", 0xA1EAFC, 3, CropType.CROP, Map.of(
        "aoa3", "item.aoa3.shyrestone_ingot"
    ));
    @Nullable
    public static final CompatCrop SILICON = Crops.compatCrop("silicon", "#c:silicon", 0x66546D, 3, CropType.CROP, Map.of(
        "ae2", "item.ae2.silicon"
    ));
    @Nullable
    public static final CompatCrop STEEL = Crops.compatCrop("steel", "#c:ingots/steel", 0xA0A0A0, 3, CropType.CROP, Map.of(
        "ad_astra", "item.ad_astra.ingot_steel",
        "mekanism", "item.mekanism.ingot_steel"
    ));
    @Nullable
    public static final CompatCrop OSMIUM = Crops.compatCrop("osmium", "#c:ingots/osmium", 0x9EB1C8, 3, CropType.CROP, Map.of(
        "mekanism", "item.mekanism.ingot_osmium"
    ));
    @Nullable
    public static final CompatCrop PLATINUM = Crops.compatCrop("platinum", "#c:ingots/platinum", 0x9EB1C8, 3, CropType.CROP, Map.of(
        "modern_industrialization", "item.modern_industrialization.platinum_ingot"
    ));
    @Nullable
    public static final CompatCrop ROSITE = Crops.compatCrop("rosite", "#c:ingots/rosite", 0xF16B59, 3, CropType.CROP, Map.of(
        "aoa3", "item.aoa3.rosite_ingot"
    ));
    @Nullable
    public static final CompatCrop SKELETAL = Crops.compatCrop("skeletal", "#c:ingots/skeletal", 0xB3A997, 3, CropType.CROP, Map.of(
        "aoa3", "item.aoa3.skeletal_ingot"
    ));
    @Nullable
    public static final CompatCrop TIN = Crops.compatCrop("tin_conventional", "#c:ingots/tin_conventional", 0xE3E3E0, 3, CropType.CROP, Map.of(
        "modern_industrialization", "item.modern_industrialization.tin_ingot",
        "mekanism", "item.mekanism.ingot_tin"
    ));
    @Nullable
    public static final CompatCrop TITANIUM = Crops.compatCrop("titanium", "#c:ingots/titanium", 0x8F9E9A, 3, CropType.CROP, Map.of(
        "modern_industrialization", "item.modern_industrialization.titanium_ingot"
    ));
    @Nullable
    public static final CompatCrop TUNGSTEN = Crops.compatCrop("tungsten", "#c:ingots/tungsten", 0x8F9E9A, 3, CropType.CROP, Map.of(
        "modern_industrialization", "item.modern_industrialization.tungsten_ingot"
    ));
    @Nullable
    public static final CompatCrop URANIUM = Crops.compatCrop("uranium_conventional", "#c:ingots/uranium", 0x32CE00, 3, CropType.CROP, Map.of(
        "modern_industrialization", "item.modern_industrialization.uranium_ingot",
        "mekanism", "item.mekanism.ingot_uranium"
    ));
    @Nullable
    public static final CompatCrop VARSIUM = Crops.compatCrop("varsium", "#c:ingots/varsium", 0xDABF59, 3, CropType.CROP, Map.of(
        "aoa3", "item.aoa3.varsium_ingot"
    ));
    @Nullable
    public static final CompatCrop ZINC = Crops.compatCrop("zinc", "#c:ingots/zinc", 0xEDEEEC, 3, CropType.CROP, Map.of(
        "create", "item.create.zinc_ingot"
    ));

    public static void init() {
    }
}
