...
public class DumpCommand {
    private static final LiteralArgumentBuilder<CommandSourceStack> DUMP = Commands.literal("dump").requires(s -> s.hasPermission(2)).executes(context -> dumpAll(context.getSource()::sendSuccess, false)).then(Commands.argument("crop", StringArgumentType.greedyString()).suggests((context, builder) -> Crops.suggestCrops(builder)).executes(context -> {
        ...
        if (CropFileHandler.dumpCrop(crop)) {
            ...
            if (openFile) {
                nameComponent.withStyle(
                    CommonCommandRoot.openFile(CropariaIf.CONFIG.getDumpPath().resolve(name + ".json").toString())
                ).withStyle(CommonCommandRoot.blockMouseBehavior());
            }
