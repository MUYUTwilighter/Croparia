...
public class Crops {
    ...
    public static @NotNull Crop vanilla(
        ...
        if (CropariaIf.CONFIG.inBlacklist(name, "minecraft")) {
            CropariaIf.LOGGER.info("Skipped vanilla crop {} due to blacklist", name);
        } else {
            recordBuiltin(crop);
        }
        return crop;
    ...
    public static final Crop GRASS = vanilla("grass", Objects.requireNonNull(Items.GRASS.arch$registryName()).toString(), 0x820510b, 1, CropType.NATURE, Items.GRASS.getDescriptionId());
    ...
    public static final Crop SCUTE = vanilla("scute", Objects.requireNonNull(Items.SCUTE.arch$registryName()).toString(), 0x47bf4a, 2, CropType.ANIMAL, Items.SCUTE.getDescriptionId());
