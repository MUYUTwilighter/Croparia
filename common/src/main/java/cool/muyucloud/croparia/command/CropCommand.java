...
public class CropCommand {
    private static LiteralArgumentBuilder<CommandSourceStack> CROP = Commands.literal("crop").requires(s -> s.hasPermission(2)).then(
        Commands.argument("name", StringArgumentType.greedyString()).suggests((context, builder) -> {
    ...
    public static MutableComponent buildReport(@NotNull Crop crop) {
        ...
        MutableComponent material = Component.translatable(
            ...
            Component.literal(crop.serializeMaterial()).withStyle(
                ServerRoot.suggestCommand("/give @s %s", crop.getMaterialItem().arch$registryName())
            ).withStyle(
                ServerRoot.hoverItem(crop.getMaterialItem().arch$registryName())
            ).withStyle(ServerRoot.inlineMouseBehavior())
        );
        MutableComponent tier = Component.translatable(
            "commands.croparia.crop.tier",
            Component.literal(crop.getTier() + "").withStyle(
                ServerRoot.suggestCommand("/give @s %s", CropariaItems.getCroparia(crop.getTier()).getId())
            ).withStyle(
                ServerRoot.hoverItem(CropariaItems.getCroparia(crop.getTier()).get())
            ).withStyle(ServerRoot.inlineMouseBehavior())
        );
        MutableComponent color = Component.translatable(
            "commands.croparia.crop.color",
            Component.literal(crop.serializeColor()).withColor(crop.getColor())
        );
        ...
        MutableComponent seed = Component.translatable(
            ...
            Component.literal(crop.getSeedId().toString()).withStyle(
                ServerRoot.suggestCommand("/give @s %s", crop.getSeedId().toString())
            ).withStyle(
                ServerRoot.hoverItem(crop.getSeedItem())
            ).withStyle(ServerRoot.inlineMouseBehavior())
        );
        MutableComponent fruit = Component.translatable(
            ...
            Component.literal(crop.getFruitId().toString()).withStyle(
                ServerRoot.suggestCommand("/give @s %s", crop.getFruitId().toString())
            ).withStyle(
                ServerRoot.hoverItem(crop.getFruitItem())
            ).withStyle(ServerRoot.inlineMouseBehavior())
        );
