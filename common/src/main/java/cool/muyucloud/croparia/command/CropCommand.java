...
public class CropCommand {
    ...
    public static MutableComponent buildReport(@NotNull Crop crop) {
        ...
        MutableComponent translation = new TranslatableComponent("commands.croparia.crop.translation", new TranslatableComponent(crop.getTranslationKey()).withStyle(ServerCommandRoot.hoverText(crop.getTranslationKey())).withStyle(ServerCommandRoot.copyText(crop.getTranslationKey())));
        MutableComponent material = new TranslatableComponent("commands.croparia.crop.material", new TextComponent(crop.serializeMaterial()).withStyle(ServerCommandRoot.suggestCommand("/give @s %s", crop.getMaterialItem().arch$registryName())).withStyle(ServerCommandRoot.hoverItem(crop.getMaterialItem().arch$registryName())).withStyle(ServerCommandRoot.inlineMouseBehavior()));
        MutableComponent tier = new TranslatableComponent("commands.croparia.crop.tier", new TextComponent(crop.getTier() + "").withStyle(ServerCommandRoot.suggestCommand("/give @s %s", CropariaItems.getCroparia(crop.getTier()).getId())).withStyle(ServerCommandRoot.hoverItem(CropariaItems.getCroparia(crop.getTier()).get())).withStyle(ServerCommandRoot.inlineMouseBehavior()));
        MutableComponent color = new TranslatableComponent("commands.croparia.crop.color", new TextComponent(crop.serializeColor()).withStyle(ServerCommandRoot.color(crop.getColor())));
        MutableComponent type = new TranslatableComponent("commands.croparia.crop.type", crop.getType().getModelName());
        MutableComponent seed = new TranslatableComponent("commands.croparia.crop.seed", new TextComponent(crop.getSeedId().toString()).withStyle(ServerCommandRoot.suggestCommand("/give @s %s", crop.getSeedId().toString())).withStyle(ServerCommandRoot.hoverItem(crop.getSeedItem())).withStyle(ServerCommandRoot.inlineMouseBehavior()));
        MutableComponent fruit = new TranslatableComponent("commands.croparia.crop.fruit", new TextComponent(crop.getFruitId().toString()).withStyle(ServerCommandRoot.suggestCommand("/give @s %s", crop.getFruitId().toString())).withStyle(ServerCommandRoot.hoverItem(crop.getFruitItem())).withStyle(ServerCommandRoot.inlineMouseBehavior()));
        MutableComponent cropBlock = new TranslatableComponent("commands.croparia.crop.cropBlock", new TextComponent(crop.getBlockId().toString()).withStyle(ServerCommandRoot.suggestCommand("/setblock ~ ~ ~ %s[age=7]", crop.getBlockId().toString())).withStyle(ServerCommandRoot.hoverText(crop.getCropBlock().getName())).withStyle(ServerCommandRoot.inlineMouseBehavior()));
