...
public class ElementalWater extends Item {
    public ElementalWater() {
        super(new Item.Settings().registryKey(RegistryKey.of(RegistryKeys.ITEM, Identifier.of("croparia", "elemental_water"))));
    }
        ...
        if (!context.getWorld().isClient) {
            ...
            if (state.getBlock() instanceof LeveledCauldronBlock) {
                ...
                if (block.isFull(state)) {
                    world.setBlockState(pos, (BlockState)BlockInit.WATER_CAULDRON.getDefaultState().with(LeveledCauldronBlock.LEVEL, 3));
