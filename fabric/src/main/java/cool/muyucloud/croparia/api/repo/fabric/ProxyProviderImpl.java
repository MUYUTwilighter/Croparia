...
public class ProxyProviderImpl {
    ...
    public static void registerItem(ProxyProvider<ItemSpec> provider, Block... blocks) {
        for (Block block : blocks) {
            if (!(block instanceof BaseEntityBlock)) {
                Repo.LOGGER.debug("Registering item proxy on a non-entity block: {}, which will not work on forge", block);
            }
        }
    ...
    public static void registerFluid(ProxyProvider<FluidSpec> provider, Block... blocks) {
        for (Block block : blocks) {
            if (!(block instanceof BaseEntityBlock)) {
                Repo.LOGGER.debug("Registering fluid proxy on a non-entity block: {}, which will not work on forge", block);
            }
        }
