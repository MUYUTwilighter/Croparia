...
public class PlatformItemProxyImpl implements PlatformItemProxy {
    ...
    public StorageView<ItemVariant> get(int i) {
        int v = 0;
        Iterator<? extends StorageView<ItemVariant>> iterator = this.storage.iterator(Transaction.openOuter());
    ...
    public int size() {
        int i = 0;
        for (StorageView<ItemVariant> ignored : this.get().iterable(Transaction.openOuter())) {
            i++;
        }
        return i;
    }
    ...
    public long simConsume(int i, ItemSpec resource, long amount) {
        ...
        if (view == null) {
            ...
            try (Transaction transaction = Transaction.openOuter()) {
                ItemVariant required = FabricItemSpec.of(resource);
    ...
    public long simAccept(int i, ItemSpec resource, long amount) {
        ...
        if (!(view instanceof Storage<?> s)) {
            ...
            try (Transaction transaction = Transaction.openOuter()) {
                @SuppressWarnings("unchecked") Storage<ItemVariant> storage = (Storage<ItemVariant>) s;
                return storage.insert(FabricItemSpec.of(resource), amount, transaction);
    ...
    public long capacityFor(ItemSpec fluid) {
        ...
        long result = 0L;
        for (StorageView<ItemVariant> view : this.get().iterable(Transaction.openOuter())) {
