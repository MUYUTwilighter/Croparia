...
public class PlatformFluidProxyImpl implements PlatformFluidProxy {
    ...
    public StorageView<FluidVariant> get(int i) {
        int v = 0;
        Iterator<? extends StorageView<FluidVariant>> iterator = this.storage.iterator(Transaction.openOuter());
    ...
    public int size() {
        int i = 0;
        for (StorageView<FluidVariant> ignored : this.get().iterable(Transaction.openOuter())) {
            i++;
        }
        return i;
    }
    ...
    public long simConsume(int i, FluidSpec resource, long amount) {
        ...
        if (view == null) {
            ...
            try (Transaction transaction = Transaction.openOuter()) {
                FluidVariant required = FabricFluidSpec.of(resource);
                return view.extract(required, amount, transaction);
            }
        }
    }
    ...
    public long simAccept(int i, FluidSpec resource, long amount) {
        ...
        if (!(view instanceof Storage<?> s)) {
            ...
            try (Transaction transaction = Transaction.openOuter()) {
                @SuppressWarnings("unchecked") Storage<FluidVariant> storage = (Storage<FluidVariant>) s;
                return storage.insert(FabricFluidSpec.of(resource), amount, transaction);
    ...
    public long capacityFor(FluidSpec fluid) {
        ...
        long result = 0L;
        for (StorageView<FluidVariant> view : this.get().iterable(Transaction.openOuter())) {
